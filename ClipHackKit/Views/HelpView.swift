        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                ...
                section("Getting Started") {
                    ...
                    text("Notes typed when downloading a clip are shown under its name in the list. Point at them to read the full text. They stay with the row and are not saved to disk.")
                }
                section("Download from URL") {
                    ...
                    definition("Person in clip", "Optional. Who is speaking — it opens the notes shown on the clip's row. For X/Twitter links, ClipHack reads the name out of the post's own text when it can, and leaves the field blank when it can't. It never uses the account that posted the clip, because that is usually not the person in it.")
                    definition("Notes", "Optional free text kept with the file row. It is not written to the session's notes file. Its first line is what the clip is about, with the person in front of it; anything on the lines below is yours (timings, scratch) and rides along untouched. Drag the grip in the box's bottom-right corner to make it taller; the size is remembered. For X/Twitter post links, the post's text is fetched and pre-filled here automatically (best-effort) — edit or clear it as you like.")
                    definition("The cut", "Optional. The part of the clip you want — 1:13 to :55. Written on its own line in the session's notes file.")
                    definition("Save clip notes", "Records each download in the session's own file — HT_0379 2026-08-24/clips/HT_0379 2026-08-24.txt — one block per clip, separated by a --- rule. Inside a block: the file name, the cut if you entered one, then the source URL, with a blank line between each. The person and notes are not written, and the file name is written even when you typed it yourself. Nothing is ever appended: ClipHack rewrites the file from what it holds, so re-downloading a clip replaces its block instead of stacking another one. Text files from an earlier version are folded in the first time you open the session, and the originals are left where they are.")
                    definition("Destination", "Where downloads land. Defaults to ~/Music/ClipHack; use Change… to pick any folder — it's remembered for future downloads. If that folder is missing when you download, ClipHack asks for a new one. In a temporary session it is the Desktop.")
                ...
                section("Sessions") {
                    ...
                    definition("Temporary Session", "For a file that isn't for an episode. Downloads and processed audio go to the Desktop, no notes file is written, and the Destination and Output Directory controls are off. Your episode is left exactly as it was. Choose it again, or pick any episode, to leave; quitting leaves it too, so ClipHack always opens on your episode. If macOS has refused ClipHack access to the Desktop, allow it in System Settings ▸ Privacy & Security ▸ Files and Folders.")
